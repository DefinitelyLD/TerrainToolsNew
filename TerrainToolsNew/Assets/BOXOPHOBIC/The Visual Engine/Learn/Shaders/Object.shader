...
Shader "Hidden/The Visual Engine/Object"
{
	Properties
	{
		...
		[Space(10)][StyledTextureSingleLine] _ATexture( "A Texture", 2D ) = "white" {}
		[StyledTextureSingleLine] _NTexture( "N Texture", 2D ) = "bump" {}
		[StyledTextureSingleLine] _OTexture( "O Texture", 2D ) = "white" {}
		[StyledTextureSingleLine] _MTexture( "M Texture", 2D ) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 )
			{
				...
				float2 uv_ATexture = packedInput.ase_texcoord5.xy * _ATexture_ST.xy + _ATexture_ST.zw;
				
				float2 uv_NTexture = packedInput.ase_texcoord5.xy * _NTexture_ST.xy + _NTexture_ST.zw;
				float3 unpack7 = UnpackNormalScale( tex2D( _NTexture, uv_NTexture ), _Normal );
				unpack7.z = lerp( 1, unpack7.z, saturate(_Normal) );
				
				float2 uv_MTexture = packedInput.ase_texcoord5.xy * _MTexture_ST.xy + _MTexture_ST.zw;
				float4 tex2DNode8 = tex2D( _MTexture, uv_MTexture );
				
				float2 uv_OTexture = packedInput.ase_texcoord5.xy * _OTexture_ST.xy + _OTexture_ST.zw;
				

				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;

				surfaceDescription.BaseColor = ( _Color * tex2D( _ATexture, uv_ATexture ) ).rgb;
				...
				surfaceDescription.Occlusion = tex2D( _OTexture, uv_OTexture ).g;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float2 uv_ATexture = packedInput.ase_texcoord2.xy * _ATexture_ST.xy + _ATexture_ST.zw;
				
				float2 uv_NTexture = packedInput.ase_texcoord2.xy * _NTexture_ST.xy + _NTexture_ST.zw;
				float3 unpack7 = UnpackNormalScale( tex2D( _NTexture, uv_NTexture ), _Normal );
				unpack7.z = lerp( 1, unpack7.z, saturate(_Normal) );
				
				float2 uv_MTexture = packedInput.ase_texcoord2.xy * _MTexture_ST.xy + _MTexture_ST.zw;
				float4 tex2DNode8 = tex2D( _MTexture, uv_MTexture );
				
				float2 uv_OTexture = packedInput.ase_texcoord2.xy * _OTexture_ST.xy + _OTexture_ST.zw;
		...
		Pass
		{
						 ...
						 )
			{
				...
				float2 uv_NTexture = packedInput.ase_texcoord3.xy * _NTexture_ST.xy + _NTexture_ST.zw;
				float3 unpack7 = UnpackNormalScale( tex2D( _NTexture, uv_NTexture ), _Normal );
				unpack7.z = lerp( 1, unpack7.z, saturate(_Normal) );
				
				float2 uv_MTexture = packedInput.ase_texcoord3.xy * _MTexture_ST.xy + _MTexture_ST.zw;
				float4 tex2DNode8 = tex2D( _MTexture, uv_MTexture );
		...
		Pass
		{
					 ...
					 )
			{
				...
				float2 uv_ATexture = packedInput.ase_texcoord7.xy * _ATexture_ST.xy + _ATexture_ST.zw;
				
				float2 uv_NTexture = packedInput.ase_texcoord7.xy * _NTexture_ST.xy + _NTexture_ST.zw;
				float3 unpack7 = UnpackNormalScale( tex2D( _NTexture, uv_NTexture ), _Normal );
				unpack7.z = lerp( 1, unpack7.z, saturate(_Normal) );
				
				float2 uv_MTexture = packedInput.ase_texcoord7.xy * _MTexture_ST.xy + _MTexture_ST.zw;
				float4 tex2DNode8 = tex2D( _MTexture, uv_MTexture );
				
				float2 uv_OTexture = packedInput.ase_texcoord7.xy * _OTexture_ST.xy + _OTexture_ST.zw;
