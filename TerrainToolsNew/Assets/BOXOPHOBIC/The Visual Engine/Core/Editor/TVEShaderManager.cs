...
namespace TheVisualEngine
{
    public class TVEShaderManager : EditorWindow
    {
        ...
        void StyledShader(int index)
        {
            ...
            //}
            //else if (shaderType.Contains("The Visual Engine "))
            //{
            //    if (AssetDatabase.GetAssetPath(shader).Contains("Lite"))
            //    {
            //        shaderType = "Lite";
            ...
            var popupRect = new Rect(GUI_HALF_EDITOR_WIDTH, lastRect.y + 3, lastRect.width / 2 - 28, lastRect.height);
            var selectionRect = new Rect(lastRect.x, lastRect.y, GUI_HALF_EDITOR_WIDTH, lastRect.height);
            var duplicateRect = new Rect(lastRect.width - 8, lastRect.y + 3, 18, 18);
