...
namespace TheVisualEngine
{
    public class TVEAssetConverter : EditorWindow
    {
        ...
        int collectBackups = 1;
        int collectLocking = 0;
        ...
        string assetFolder = "Assets/BOXOPHOBIC/The Visual Engine";
        ...
        void OnEnable()
        {
            ...
            assetFolder = TVEUtils.GetAssetFolder();
            ...
            projectPipeline = TVEUtils.GetProjectPipeline();
        ...
        void DrawConversionButtons()
        {
            ...
            else
            {
                ...
                //GUI.color = new Color(0.0f, 1.0f, 1.0f);
        ...
        void StyledPrefab(TVEPrefabData prefabData)
        {
            ...
            if (prefabData.status == TVEPrefabMode.Supported)
            {
                ...
                {
                    GUI.Label(iconExtra, EditorGUIUtility.IconContent("console.warnicon.sml"));
                    GUI.Label(iconExtra, new GUIContent("", "The prefab has rotated gameobjects or the meshes are set as Z-up which might cause issues because the shaders expect meshes in Y-up axis! The converter can bake the axis rotations if the Transforms Mode is set to Transform To World Space under Model Settings!"));
                }
            }
            ...
            if (prefabData.status == TVEPrefabMode.ConvertedAndLocked)
            {
                ...
                GUI.Label(iconLock, new GUIContent("", "The prefab cannot be reverted or reconverted because the prefab is locked!"));
        ...
        void ConvertPrefab(TVEPrefabData prefabData)
        {
            ...
            {
                prefabInstance = Instantiate(prefabData.prefabObject);
            }
            else
            {
                prefabInstance = (GameObject)PrefabUtility.InstantiatePrefab(prefabData.prefabObject);
            }

            prefabInstance.name = prefabData.prefabObject.name;
        ...
        void RevertPrefab(TVEPrefabData prefabData)
        {
            ...
            {
                prefabInstance = (GameObject)PrefabUtility.InstantiatePrefab(prefabBackup);
        ...
        TVEPrefabData GetPrefabAttributes(TVEPrefabData prefabData)
        {
            ...
            //if (prefabData.prefabObject.transform.rotation.eulerAngles.x != 0)
            //{
            ...
            for (int i = 0; i < gameObjects.Count; i++)
            {
                ...
                if (PrefabUtility.GetNearestPrefabInstanceRoot(gameObject) != PrefabUtility.GetNearestPrefabInstanceRoot(prefabData.prefabObject))
                {
                    gameObjects[i] = null;
        ...
        void DisableInvalidGameObjectsInPrefab()
        {
            for (int i = 0; i < gameObjectDataSet.Count; i++)
            {
                ...
                if (PrefabUtility.GetNearestPrefabInstanceRoot(gameObject) != PrefabUtility.GetNearestPrefabInstanceRoot(prefabInstance))
                {
                    gameObject.SetActive(false);
                }
            }
                ...
                if (useLine)
                {
                    ...
                    if (presetLines[i].StartsWith("Material") && doPostProcessing)
                    {
                        ...
                        else if (type == "SET_TEX")
                        {
                            ...
                            instanceMaterial.SetTexture(set, Resources.Load<Texture>(val));
                        }
                        else if (type == "SET_TEX_FROM_CORE")
                        {
                            val = val.Replace("__", " ");
                            var path = assetFolder + "/" + val;

                            instanceMaterial.SetTexture(set, AssetDatabase.LoadAssetAtPath<Texture>(path));
                        }
                        else if (type == "SET_TEX_BY_PATH")
                        {
                            val = val.Replace("__", " ");

                            instanceMaterial.SetTexture(set, AssetDatabase.LoadAssetAtPath<Texture>(val));
                        }
                        ...
                        else if (type == "COPY_FLOAT")
                        {
                            if (material.HasFloat(src))
                            {
                                ...
                                value = ApplyFloatActions(action, value);
                        ...
                        else if (type == "COPY_VECTOR_X_AS_FLOAT")
                        {
                            if (material.HasProperty(src))
                            {
                                ...
                                value.x = ApplyFloatActions(action, value.x);
                        ...
                        else if (type == "COPY_VECTOR_Y_AS_FLOAT")
                        {
                            ...
                            {
                                var value = material.GetVector(src);
                                value.y = ApplyFloatActions(action, value.y);
                        ...
                        else if (type == "COPY_VECTOR_Z_AS_FLOAT")
                        {
                            ...
                            {
                                var value = material.GetVector(src);
                                value.z = ApplyFloatActions(action, value.z);
                        ...
                        else if (type == "COPY_VECTOR_W_AS_FLOAT")
                        {
                            ...
                            {
                                var value = material.GetVector(src);
                                value.w = ApplyFloatActions(action, value.w);
                    ...
                    if (presetLines[i].StartsWith("Texture") && doPostProcessing)
                    {
                        ...
                        if (propIsValidTexture)
                        {
                            ...
                            if (presetLines[i].Contains("NONE"))
                            {
                                action0Index = 0;
                                action1Index = 0;
                                action2Index = 0;
                    ...
                    if (presetLines[i].StartsWith("Utility"))
                    {
                        ...
                        if (type == "START_TEXTURE_PACKING" || type == "FORCE_TEXTURE_PACKING")
                        {
                            ...
                            bool validPacking = additionalTexturesFound || additionalCoordsFound || additionalChannelsFound || additionalAction0Found;
        ...
        float ApplyFloatActions(string action, float value)
        {
            if (action == "ACTION_ONE_MINUS")
            {
                value = 1 - value;
            ...
            else if (action == "ACTION_MULTIPLY_05")
            {
                value = value * 0.5f;
            }
        ...
        string GetConvertedAssetName(Object asset, string type, bool shareCommonAssets)
        {
            ...
            {
                name = name + " " + prefabInstance.name;
            }

            var saveFormat = outputDataFormat;
            saveFormat = saveFormat.Replace("BASENAME", prefabInstance.name);
        ...
        bool GetConditionFromLine(List<bool> usePresetLines, string line, Material material)
        {
            ...
            if (line.StartsWith("if"))
            {
                ...
                {
                    var path = AssetDatabase.GetAssetPath(prefabInstance).ToUpperInvariant();
